...
test.describe(`Storefront Suite Definitiva – ${BASE_URL}`, () => {
  ...
  test('STF-01: Home - Logo, Navegación y Scroll Footer', async ({ page }) => {
    ...
    const logo = page.locator('header img-INEXISTENTE-INEXISTENTE, [class*="logo"], header a').first();
  ...
  test('STF-04: PDP & Checkout - Flujo Completo Suscripción, Compra Única y Carrito', async ({ page }) => {
    ...
    if (await subscribeLabel.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      // Confirmar paso a checkout o carrito
      await page.goto(`${BASE_URL}/checkout`, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch(() => null);
    ...
    // 6. Confirmar la carga del formulario de carrito / checkout
    const bodyTextFinal = await page.locator('body').innerText();
    expect(bodyTextFinal).not.toContain('Error 500');
  });
